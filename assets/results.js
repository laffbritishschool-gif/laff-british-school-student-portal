...
async function loadResults(){
 ...
 try{
  ...
  const body=results.map((r,i)=>{const grade=esc(r.grade||'—');const cls=grade==='A'?'grade-a':grade==='B'?'grade-b':'';return `<tr><td>${i+1}</td><td><b>${esc(r.subjects?.name||'Subject')}</b></td><td>${Number(r.ca_score||0)}</td><td>${Number(r.exam_score||0)}</td><td><b>${Number(r.total||0)}</b></td><td><span class="grade ${cls}">${grade}</span></td><td>${esc(r.grade_point??'—')}</td><td>${esc(r.teacher_remark||'—')}</td></tr>`}).join('');html('resultBody',body);show('resultTableWrap');show('summaryRow');show('remarks');text('average',`${avg.toFixed(1)}%`);text('overallGrade',avg>=70?'A':avg>=60?'B':avg>=50?'C':avg>=45?'D':avg>=40?'E':'F');text('position',position?`${position}${position===1?'st':position===2?'nd':position===3?'rd':'th'}`:'—');text('subjectsCount',results.length);text('teacherRemark',teacherRemarks[0]||'—');text('principalRemark',principalRemarks[0]||'—');state.remove();
...
document.getElementById('printResult')?.addEventListener('click',()=>window.print());loadResults();
