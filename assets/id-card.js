...
async function gate(){
 ...
 const section=document.querySelector('.card-section'); if(!section)return false;
...
async function loadCard(){
 try{
  ...
  const [{data:student},{data:school},{data:sessionRow},{data:enrollment},{data:card}]=await Promise.all([
   supabase.from('students').select('id,student_id,first_name,middle_name,last_name,school_email,photo_url,status').eq('user_id',session.user.id).maybeSingle(),
   ...
   supabase.from('enrollments').select('id,classes(name)').eq('student_id',session.user.id).eq('status','ACTIVE').limit(1).maybeSingle(),
   supabase.from('id_cards').select('card_number,issued_at,expires_at,is_active').eq('student_id',(await supabase.from('students').select('id').eq('user_id',session.user.id).maybeSingle()).data?.id||'').eq('is_active',true).limit(1).maybeSingle()
  ]);
  if(!student)return;
